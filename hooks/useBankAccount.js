...
function normalizeAccount(account) {
  return {
    ...
    accountName:
      account.accountName || account.name || account.holderName || "",
    accountNumber:
      account.accountNumber || account.registeredAccountNumber || "",
...
function pickPrimaryAccount(accounts) {
  ...
  return (
    normalizedAccounts.find((account) => account.isActive === true) ||
    normalizedAccounts.sort(
      (first, second) =>
        getMillis(second.updatedAt || second.createdAt) -
        getMillis(first.updatedAt || first.createdAt),
    )[0] ||
    emptyBankAccount
  );
}
